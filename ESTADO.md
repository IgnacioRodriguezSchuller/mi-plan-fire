...
10. **[Cartel Fase 2 · casi cerrada]** Primitivas (S6); chrome de sección (S8/S9); **eyebrows del producto migradas a serif** — el primitivo `Label` y las eyebrows inline de tarjeta (TU PATRIMONIO, PARADO/INVERTIDO, «Tu mes», stats…) ahora son serif itálica (una voz; verificado en Hoy/Seguimiento/SinMiPlanModal). **Cola menor pendiente:** ~40 labels de baja visibilidad (barra «PLAN»/«% inversión», chips «EN CAMINO», «× CERRAR», leyendas de gráfico, labels sobre tarjeta oscura, labels con color dinámico `tier.color`) + GX6 (leyenda «los números subrayados los pones tú» fuera de Proyección, depende de migrar los editables al estilo dashed del Cartel).
