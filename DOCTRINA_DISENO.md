...
- **M2 "Hacia dónde puedes ir" — cards verdes.** Fondo `linear-gradient(180deg, T.greenSoft, transparent)`, `border: T.green`. Gancho (todos los perfiles): dos líneas `T.display` 600, la 2ª en `T.accent`. Cuerpo (perfiles que aportan): monedas + cierre con la renta de retiro real en `T.amber`, adaptado al veredicto de suficiencia. **VERACIDAD — UN SOLO NÚMERO (innegociable)**: `ratio = finalReal / aportadoReal`, MISMO modelo de aporte (creciente, projectV2) y MISMAS unidades (`€` de hoy) que el M1; `aportadoReal` = suma de cada aporte mensual deflactado. **Las monedas Y la frase derivan SIEMPRE de ese mismo ratio**, nunca se contradicen, nunca se redondea hacia un mensaje bonito.
  - **Monedas abstractas, sin tope arriba ni abajo**: izq 1 moneda `T.ink` (lo que pones, etiqueta = aportadoReal) → flecha `T.amber` → der el ratio dibujado: `floor(ratio)` monedas `T.green` llenas (bucle, soporta ×5, ×8, ×12…) + 1 moneda de fracción con `opacity = max(decimal, 0.15)` (etiqueta = finalReal). Robustez: `flex-wrap` a varias filas + reescala del disco si hay muchas → nunca desborda en 375. **Guarda de dato degenerado** (NaN/Infinity, o `aportadoReal≈0` → ratio ≥ 30): no se pintan decenas de monedas, se oculta el bloque (como perfil sin aporte) preservando la renta. No es un tope al mensaje, es robustez.
  - **Frase adaptada al mismo ratio** (línea 2 nunca afirma un múltiplo que las monedas no dibujen): `≥4.5` → "multiplica por {round(ratio)}…" (número real, jamás "casi cuadruplica" cuando es más); `3.5–4.5` → "casi cuadruplica…"; `2.5–3.5` → "casi triplica…"; `1.8–2.5` → "más que duplica…"; `1.3–1.8` → "multiplica lo que pones por {X,X}"; `<1.3` → ángulo-futuro ("Esto no ha hecho más que empezar: / cada año que pase, multiplica más.") en vez de presumir un múltiplo pobre. Perfil A (no aporta): sin cuerpo de monedas. **Una cifra que engaña es un fallo grave, no licencia de marketing: no hay umbral ni "caso estándar" — el dibujo dice la verdad, sea cual sea.**
- **M3 "Tu ruta" — card naranja + hitos.** Card `linear-gradient(180deg, T.accentSoft, transparent)`, `border: T.accent`: encabezado "Fase n de 5 · nombre" (`T.accent`), barra de 5 tramos por estado (completada `T.green` / no-aplica `T.lineSoft` / activa `T.accent` / futura `T.line` .5), 5 pestañas clicables (marcador circular + nombre corto; la activa resaltada con borde acento + `T.paper`) y un panel de detalle **integrado** (borderTop, hereda el difuminado — no una caja blanca flotando). **Hitos FUERA de la card**, fila de 3: "tu dinero te adelanta" (primera edad en que el rendimiento anual de la cartera supera al aporte de ese año — mismo modelo de aporte que el M2; fallback honesto si no aporta/no cruza), la edad de libertad en `T.green` (excepción declarada, ver §2 KPI hero), y la jubilación legal.
