...
### U2 · Iconos de cada tipo de FIRE sobre la curva de «Tu línea de vida» (ítem 1)
- **Cambio** (`LifeChart`, `ui/cartel.jsx` + `ScreenProyeccion`, `screens`): los marcadores de la curva pasan de circulitos genéricos a **el glifo de cada tipo de FIRE** (lean=círculo, coast=onda, fat=diana) pintado sobre la línea en su edad (`ptAt(age)`), en su color, con un halo de fondo para que despegue de la línea. El **FIRE pleno** sigue siendo el **★** del cruce. Fuente única: nueva constante exportada **`FIRE_GLYPHS`** en `cartel.jsx`, usada **por la curva y por las tarjetas** (antes los glifos estaban duplicados inline en las tarjetas) → icono de curva e icono de tarjeta idénticos. `milestones` pasa a llevar `{ age, color, icon }`.
- **No tocado**: motor, datos; los tipos fuera del rango visible de la curva simplemente no se pintan (correcto). **Verificación**: navegador — en Proyección, la curva muestra el glifo de Lean (color accent) centrado en su edad con halo + el ★ del FIRE pleno; las tarjetas usan el mismo glifo. Build OK; content/state PASS; lib 13 / tokens 2.

### U3 · Gráfica del reparto del ingreso: palabras, tipografía, tamaños + verde más vivo (ítem 3)
- **Contexto**: el dueño aclaró que la gráfica como tal está bien; el problema eran las palabras, la tipografía y los tamaños, y el verde podía ser más vivo.
- **Cambio** (`FlowTimelineCard`, `charts/index.jsx`): **copy** más corto y claro («Cómo se reparte tu ingreso»; descripción de una frase «En N años invertirás unos X, de un ingreso total de Y», antes verbosa de dos cifras/mes + total). **Tipografía/tamaños**: la descripción sube de `T.size.caption` (13px) a `T.size.body` y la leyenda de `T.size.eyebrow` (11px) a `T.size.caption` (con chips algo mayores). **Color**: el verde de inversión más **vivo** (degradado `reparto-invest` 0,34→0,58 arriba / 0,06→0,14 abajo; stroke 2→2,5). La estructura de la gráfica (área apilada) **no se toca**.
- **Invariante**: verde = `T.green` a mayor opacidad (token, no literal). **Verificación**: navegador (Seguimiento) — título sin «en el tiempo», descripción de una frase a 16px (antes 13px), área de inversión más viva (opacidad 0,58). Build OK; content/state PASS; lib 13 / tokens 2.

### U4 · «Tu situación económica»: editor inline completo en la card (ítem 4)
- **Cambio** (`ActualLifeOnboarding`, `screens`): nueva prop **`inline`**. Cuando `inline`: no hay overlay fijo (ni bloqueo de scroll ni Escape), se muestran las **tres secciones a la vez** (gastos + hipoteca + asignación, sin pasos ni indicador «Paso N de 4» ni «✕ CERRAR» ni paso de confirmación) y el pie es un único **«Guardar cambios»** que llama a `finish()`/`onComplete` (deshabilitado con aviso hasta que la asignación sume 100 %). El modo modal (default `inline:false`) queda **intacto** para sus otros usos (p.ej. `ScreenSinMiPlan`). Reusa `ExpenseRow`/`AllocRow` y la validación.
- **Cambio** (`ScreenAjustes`): la card «Tu situación económica» sustituye el botón «Editar… →» + modal por **`<ActualLifeOnboarding inline onComplete={updatePlan(actualLife)} />`** abierto dentro de la card → más visible e incita a usarlo. Retirado el estado `showEditLife` y el montaje del modal.
- **No tocado**: el `data`-local + `commit()` del asistente, el shape persistido (`actualLife`), el motor. **Verificación**: navegador (Datos) — los campos (gastos/hipoteca/asignación) salen abiertos, sin chrome de modal; poner asignación a 100 % habilita «Guardar cambios»; al guardar, `plan.actualLife` queda `completed:true` con la asignación. Build OK; content/state PASS; lib 13 / tokens 2; consola limpia.

## ───────────── Deep-link: pre-rellenado del perfil desde las calculadoras (2026-06-21) ─────────────
> Pieza de conversión del embudo calculadoras.miplanfire.com → app. Quien llega desde una
> calculadora «no empieza de cero»: la app lee sus datos de la URL y pre-rellena el perfil. Un commit.

### Causa raíz / objetivo
- Las calculadoras enlazan con los datos en la query (`?utm_source=calculadora&calc=<slug>&gastoMensual=…&swr=…&patrimonio=…&aporteMensual=…&rent=…`). La app los **ignoraba** → el usuario rehacía a mano lo que ya había metido en la calculadora.

### Cambio
- **Módulo nuevo y aislado `app/src/lib/deeplink.js`**: `parseDeeplinkParams(search)` (pura, allowlist + validación numérica + rango razonable; descarta cada param inválido por separado) y `applyDeeplinkAtStartup()` (orquestación). **Mapeo**: `patrimonio→plan.capital`, `rent→plan.annualReturn`, `swr→plan.withdrawalRate` **+ `withdrawalRateAuto=false`** (si no, `effectiveWithdrawalRate` deriva la tasa del horizonte e ignora la entrante), `aporteMensual→savingSegments[]` (tramo `fixed`) + espejo `monthlyPlanned`, `gastoMensual→actualLife.expenses.other` **+ `completed=true`** (si no, `monthlyLifeNow` cae a `max(0, ingreso−aporte)` y «Tu número» no refleja el gasto). Derivado por decisión del dueño: **`incomeSegments = gasto + aporte`** para que la tasa de ahorro y «reparto del ingreso» queden coherentes.
- **Saltar onboarding** en el sembrado (`onboardingComplete/hasSeenLandingPreOnboarding/landingSeen=true`, como `seedAlex`): sin esto, el handler de fin de onboarding (`update({plan:{…}})`, merge superficial de primer nivel) **reescribiría el plan entero** y borraría lo pre-rellenado.
- **Política**: pre-rellena **solo si no hay estado previo** (`loadAccountsData()==null`, cubre ambas claves); nunca sobrescribe. **Limpia la URL siempre** (haya sembrado o no) vía `history.replaceState`, borrando solo la allowlist y preservando ruta/hash/params ajenos → no deja datos personales en la barra.
- **Enganche único** en `app/src/main.jsx`: `applyDeeplinkAtStartup()` **antes** de `createRoot().render()` (fuera de React, 1 vez), de modo que el inicializador eager de `StateProvider` ya lea localStorage sembrado. Funciona también en el build single-file.
- **Nota de bienvenida** en `ScreenHoy` (`screens`): si llegamos desde una calculadora, card sutil estilo `NextStep` (franja `T.accent` + kicker «Desde la calculadora» + prosa serif + «Entendido»), **descartable y transitoria** (señal en memoria en `deeplink.js`: `getDeeplinkWelcome`/`dismissDeeplinkWelcome`; no persiste, no sobrevive a recargar). Sin gamificación.

### No tocado
- Firmas de `projectV2`/`runMonteCarlo`; `migrateToV2`; claves localStorage (`miplan.state.v1`/`miplan.accounts.v1`) y shape persistido (`schemaVersion 2`, `isPro`). Cero red. Color solo por tokens. Sin emojis nuevos. El sembrado solo **escribe** estado con la forma que ya producen `emptyState`/`initialAccountsData`.
- **Seguridad** (verificada): `actualLife.completed=true` con allocation a cero es seguro — `computeEffectiveCapitalReturn` devuelve `null` → el capital crece al `annualReturn` normal (no al 0 %).

### Verificación
- **Verificador nuevo `scripts/verify-deeplink.mjs`** (estilo `verify-state`: polyfill localStorage/window/history, mock Date/RNG): **8/8 PASS** — parseo, descarte de inválidos, siembra correcta (campos + skip-onboarding + URL limpia + bienvenida), no-sobrescritura con estado previo, limpieza preservando ajenos+hash, dismiss, control negativo.
- **Runtime** (localhost, localStorage limpio, `?calc=regla-del-4-por-ciento&gastoMensual=2000&swr=3.5&patrimonio=20000&aporteMensual=500&rent=5`): aterriza en el panel (no onboarding), URL → `/`, nota visible. Métricas (computadas como `useDerived`): `currentPortfolio=20000`, `income=2500`, `aporte=500`, **tasa de ahorro 20 %**, `monthlyLife=2000`, `withdrawalRate=3.5` (no la auto), **`fiTarget=685.714€`**, `effReturn=null` (→ crece al 5 %), todo finito (sin NaN/Infinity), consola limpia. Con estado previo → no se sobrescribe (capital intacto), URL igualmente limpia. `?gallery` intacto. Build OK; content/state PASS; lib 13 / tokens 2.
  - Nota preview: los héroes `ComputedNumber` se ven «0 €» en el preview headless porque su animación va gateada por `useReveal`/IntersectionObserver (no dispara sin scroll real); los valores subyacentes son correctos (probado computando como `useDerived`).

## ───────────── Lote gráfica cartel + onboarding cartel + ahorro dinámico (2026-06-21) ─────────────
> Quinto barrido del dueño. Un commit por bloque (V1–V4).

### V1 · Gráfica de evolución de ingresos «tipo cartel» (SVG a mano)
- **Cambio** (`FlowTimelineCard`, `charts/index.jsx`): se reescribe el render de **Recharts** a **SVG hecho a mano**, con la estética editorial del producto (como `LifeChart`/`MonteCarloChart`): `viewBox 640×290`, funciones `X(age)`/`Y(€)`, baseline, **dos bandas apiladas** como paths (inversión `T.green` con degradado a la base + «para vivir» en beige `T.panel` encima), la **frontera de inversión** (línea verde) y la **línea de ingreso** (tinta) con **draw-in** (`useReveal`); etiquetas serif itálicas dentro de las bandas («inviertes» / «para vivir»), las cifras de ingreso a los extremos, eje de edad con ticks + «edad». Se conserva el titular/subtítulo (U3). **Recharts se quita SOLO de esta gráfica** (las otras 4 lo siguen usando).
- **No tocado**: el cálculo de la serie (`computeIncomeFor`/`computePlannedFor`), las demás gráficas, motor. **Verificación**: navegador (Seguimiento) — la gráfica es un `<svg viewBox="0 0 640 290">` sin `.recharts-wrapper`, con 4 paths y etiquetas serif (inviertes · para vivir · cifras de ingreso · ticks de edad), coherente con la línea de vida. Build OK; content/state PASS; lib 13 / tokens 2.

### V2 · Onboarding: pulido cartel a fondo + simetría de tramos
- **Causa raíz**: el dueño detectó incongruencias tras una limpieza anterior (T2) incompleta. (1) El `tier.message` largo (frase del tramo de ahorro) sobrevivía **solo en la rama Fijo** del paso 5 — un `replace_all` previo casó el `<div>` desnudo de la rama Proporcional pero no el envuelto en `{tier && (…)}` de la rama Fijo → **asimetría visible** entre modos. (2) Las cifras hero del wizard mezclaban registros: unas en `EditableNumber` pequeño (texto base) con la unidad en `displayLg`, otras ya en accent — incoherencia de tamaños/voz frente al resto «tipo cartel».
- **Cambio** (`screens/index.jsx`, solo onboarding):
  - **Simetría del paso 5**: se retira el `tier.message` también de la rama Fijo (igual que en Proporcional). Ahora ambas ramas tienen estructura idéntica: número hero grande + unidad + `tier.label` (una palabra, `caption`) + una sola línea-eco italica («= X/mes hoy · sube con tu sueldo» / «Hoy es un X%. No crecerá…»). `getSavingsTier` **intacto** (en verify-lib).
  - **Cifras hero unificadas a `EditableValue`** (lápiz cartel, subrayado discontinuo accent, auto-tamaño) envueltas en `span` `displayLg`: paso 2 (edad), 3 (capital), 4 (ingreso), 5 (% / aporte fijo), 7 (edad de jubilación). Antes eran `EditableNumber` de tamaño base. Queda una **jerarquía de dos registros** explícita: hero = lápiz accent grande; refinamientos de detalle (% del IPC, progresión escalonada/variable) = `EditableNumber` ink pequeño en panel.
  - **Toggle Proporcional/Fijo** del paso 5: de píldora `mono` mayúsculas a **serif itálica** (voz editorial, sin `textTransform`), coherente con el resto del cartel.
- **No tocado**: `getSavingsTier` y el motor (`lib`), el wizard de 7 pasos y sus validaciones (`canNext`), `migrateToV2`, claves localStorage, `LEARN_CORPUS`, baseline. Sin cambios de copy salvo retirar la frase duplicada.
- **Verificación**: navegador (onboarding completo, viewport 1280) — pasos 2/3/4/5/7 con cifras hero grandes accent (lápiz) + unidad muted uniforme; paso 5 **simétrico** entre Proporcional y Fijo, sin la frase de tramo colgando en una sola rama; toggle serif. Build OK; `verify-content`/`verify-state` PASS; lib 13 / tokens 2 (sin diffs nuevos; cambio solo de UI); consola sin errores; hash baseline intacto.

### V3 · Ahorro dinámico «invierte lo que te sobra» (motor + UI)
- **Causa raíz / idea del dueño**: «hago 50/50 entre gasto necesario e invertir; en 10 años esa relación cambia si la inflación sube y mi sueldo no la sigue — habría que rebalancear». Respuesta: un tercer modo de ahorro en el que el aporte **no es fijo ni un %**, sino lo que sobra del sueldo tras proteger el nivel de vida, recalculándose solo año a año.
- **Motor** (`lib/index.js`, **rama aditiva**, firmas intactas):
  - `projectV2`: nuevo `monthlyFullInflation` (inflación PLENA `plan.inflationRate`, **sin** el factor de seguimiento del salario) y rama `seg.type === 'surplus'`: `aporte(m) = max(0, totalIncome(m) − costeVida(m))`, con `costeVida(m) = seg.value × (1 + monthlyFullInflation)^m` y `totalIncome(m)` creciendo por `salaryMultiplier` (= IPC × `salaryInflationFactor`). El coste de vida sube a la inflación plena; el sueldo a su ritmo → si el sueldo no sigue al coste, el aporte real **baja**; si lo supera, **sube**. `seg.value` = coste de vida €/mes de hoy.
  - `computePlannedFor`: rama `surplus` = `max(0, ingreso(key) − seg.value)` (helper de display nominal; la inflación vive en `projectV2`). `currentMonthlyAporte` la hereda.
  - Solo se ejecuta para `'surplus'` → las salidas de los casos `percent`/`fixed` no cambian: **verify-lib sigue en 13**.
- **UI** (`screens/index.jsx`):
  - **Onboarding paso 5**: tercera opción en el toggle, **«Dinámico»**. Al elegirla por primera vez, el coste de vida se siembra a ~60 % del ingreso. El control pide el **coste de vida hoy** (€/mes, `EditableValue` lápiz) y muestra el aporte implícito (`ingreso − coste`) y su % + `tier.label`, con una **nota inline** sobria que explica el mecanismo (responde literal al ejemplo del dueño). Persiste `savingSegments:[{ type:'surplus', value: costeVida, from, to:null }]` (valor nuevo del campo `type`, aditivo).
  - **Proyección «La palanca»**: cuando el segmento activo es `surplus`, la palanca editable es el **coste de vida** (no el slider de %); muestra el aporte derivado y su % + la nota del modo dinámico. `WhatIfCard`/`applyBump`: para `surplus`, «invertir +X» = **bajar** el coste de vida en X (antes sumaba a `value`, que aquí es el coste → habría hecho lo contrario). El `CartelTramoRow` del aporte dice «Inviertes lo que te sobra».
- **Explicación**: inline (nota en onboarding + Proyección). `LEARN_CORPUS` **intacto**.
- **No tocado**: firmas `projectV2`/`runMonteCarlo`/`migrateToV2`, claves localStorage, `schemaVersion 2`, `isPro`, `getSavingsTier`, baseline. Las ramas `percent`/`fixed` del motor y la UI no cambian.
- **Verificación**: navegador (onboarding nuevo, viewport 1280) — «Dinámico» con ingreso 2.000 y coste 1.200 → «Inviertes 800 €/mes hoy — lo que te sobra, un 40 % · Muy alto»; se persiste `{type:'surplus', value:1200}`, `monthlyPlanned 800`; Proyección «La palanca · lo que te sobra» muestra el coste editable (1.200 €/mes) y «Inviertes el resto: 800 €/mes (40 %)»; consola sin errores. Build OK; `verify-content`/`verify-state` PASS; **verify-lib 13** (casos percent/fixed intactos) / tokens 2; hash baseline intacto.

### W · Legibilidad + cierre de Seguimiento + bloques cartel (sugerencias · café)
Iteración visual con el dueño sobre Seguimiento, Datos y Aprende (revisada en navegador). Un solo commit: los cambios viven entrelazados en los mismos ficheros (`screens`/`charts`/`modals`).
- **Causa raíz**: (1) texto que «ni se leía» — no por tamaño sino por `T.faint` (#968874, ~4:1 de contraste) en rótulos pequeños, y por mal uso del suelo `eyebrow` (11px, «SOLO mono uppercase») en mini-rótulos **serif itálica**; (2) Seguimiento terminaba de golpe en la gráfica, sin cierre; (3) Datos no invitaba a feedback y Aprende no tenía nota de apoyo; (4) el titular de Seguimiento era menor que el de Aprende, y el eyebrow «Aprende» iba en gris pequeño (`caption`/`faint`) frente al `SectionTag` accent del resto.
- **Gráfica «El reparto»** (`charts/index.jsx`, `FlowTimelineCard`): cabecera reescrita a **cap + titular cartel** (`SectionTag` «El reparto» + titular `displayMd` en tinta), reemplazando el `Label` apagado. Etiqueta de inicio de la línea de ingreso movida **debajo** del extremo izquierdo (la escalera de subidas salariales la atravesaba). Ejes de edad y `edad` de `T.faint`→`T.muted` y `12`→`13`; etiquetas de banda/cifras `13`→`14`.
- **Legibilidad, barrido sin tocar tokens** (decisión del dueño; `tokens/index.js` intacto → verify-tokens sigue en 2): todos los mini-rótulos **no-mono** en `eyebrow`→`caption` (≈42 en `screens`+`modals`: tags «Mes actual/Atrasado/Futuro», pill «En camino/Falta», labels «Plan/Real», «Total/Diferencia», etc.) y `T.faint`→`T.muted` en texto pequeño informativo del registro mensual, calendario e hitos. Los `eyebrow` **mono uppercase** se respetan (uso correcto del suelo).
- **Cierre de Seguimiento** (`screens/index.jsx`, `ScreenSeguimiento`, último bloque): bloque editorial cartel (mismo patrón que el cierre de Plan) — eyebrow «El siguiente paso» + hero «El plan no se decide; se sostiene.» + sub + CTAs `CartelBtn` → Proyección (primario) / Aprende (texto). Distinto del `verdictNode` táctico de arriba.
- **Hitos · empty-state** simplificado: de párrafo denso que redefinía «qué es un hito» (ya lo dice el subtítulo) a invitación corta y centrada («Tu camino aún no tiene hitos.» + ejemplos), borde discontinuo.
- **Cabeceras igualadas** (petición del dueño): titular de Seguimiento `displayLg`→`displayXxl` (iguala a Aprende); eyebrow de Aprende `caption`/`faint`→`SectionTag` accent (iguala al resto). **Unificación global** (commit siguiente): Datos y Hogar `displayLg`→`displayXxl`; saludo de Plan «Hola, {nombre}» de 28/44 a 34/56 (tamaño fijo, respeta su columna de 720). Proyección ya usaba un hero mayor (`clamp(44px, 8.5vw, 112px)`) → intacto.
- **Datos · portal de sugerencias** (`screens` `ScreenAjustes`, tras «Tus datos»): bloque **cartel** centrado (sin Card) — `SectionTag` «Sugerencias» + pregunta serif + enlace serif «Enviar una sugerencia». Nueva const exportada `SUGGEST_URL=''` en `modals/index.jsx` (cero red: `<a href>`; vacío → «· pronto»).
- **Aprende · nota de café** (`screens` `ScreenAprende`, antes del footer): nota cartel sobria clonando el patrón de donación (icono café SVG ya en uso + chips Ko-fi/GitHub con `DONATE_*_URL`; vacío → «· pronto»).
- **No tocado**: motor (`lib`), `tokens/index.js` (valores), `migrateToV2`, claves localStorage, `LEARN_CORPUS`, firmas. Sin emojis nuevos; color solo por tokens; cero red.
- **Verificación**: navegador (Seguimiento/Datos/Aprende, viewport desktop) — rótulos legibles, cierre de Seguimiento navega a `proy`/`aprender`, Datos cierra con cartel «Sugerencias · pronto», Aprende con nota de café «Ko-fi/GitHub · pronto», eyebrow «Aprende» en accent 18px, titular de Seguimiento `displayXxl` (= Aprende). Build OK; `verify-content`/`verify-state` PASS; **verify-lib 13 / verify-tokens 2** (sin diffs nuevos); hash baseline `b3ea52b1…` intacto; consola limpia.

### X · Calendario de Seguimiento · objetivo del pasado + toggle del detalle
- **Causa raíz (color)**: el calendario y la lista «mes a mes» recalculaban el objetivo con `computePlannedFor(plan, key)`, que usa los `savingSegments` con `from: todayKey()` → para meses **pasados** devolvía **0** → `actual ≥ 0` siempre **verde**, aunque el aporte fuera bajo (en la demo, los meses de 380 € sobre objetivo 400 € salían verdes en vez de naranja). El `month.planned` guardado al registrar el mes se ignoraba.
- **Cambio (color)**: el objetivo del mes ahora es `computePlannedFor(...) || month.planned || 0` (calendario rejilla + detalle, `modals/index.jsx`; `MonthRow.plannedComputed`, `screens/index.jsx`). Y si el objetivo es **0** (mes sin meta real, ni viva ni guardada) → celda **neutral** (`T.paper`/`T.ink`), **nunca verde** (no había objetivo contra el que comparar). Verificado con la demo: meses de 380 € → naranja; 400/410/420/500/600 € → verde; meses sin dato → neutral.
- **Cambio (toggle)**: clicar una celda de mes ya activa **cierra** el detalle (`setActiveKey(activeKey === month.key ? null : month.key)`), antes solo abría. Verificado: 1er clic abre, 2º clic cierra.
- **No tocado**: motor (`lib`, incl. `computePlannedFor`), datos demo (`seedAlex`/`buildDemoMonths`), `migrateToV2`, claves, baseline. Solo display en Seguimiento.
- **Verificación**: build OK; `verify-content`/`verify-state` PASS; **verify-lib 13 / verify-tokens 2** (sin diffs nuevos); hash baseline intacto; consola limpia.

### Y · Calendario recogible · Hitos desplegable abajo · Menú «¿Por qué?»
Tres arreglos de UX detectados por el dueño usando la app (interacción/contenido, no motor). Un commit: entrelazados en `screens`/`modals`/`gallery`.
- **P1 · Calendario · «recoger» periodos**: «+ 6 meses» / «+ 1 año» extendían sin vuelta atrás. Nuevo helper `trimFutureMonths()` en `ScreenMesAMes` (`screens/index.jsx`) → quita SOLO meses estrictamente futuros y vacíos (`actual==null && key > todayKey()`); conserva mes actual, pasados y cualquiera con dato. Botón **«− Recoger»** en `MonthlyCalendarModal` (`modals/index.jsx`), visible solo si hay recortables. Verificado: 20 → +1 año → 32 → Recoger → 8 (datos intactos).
- **P2 · Hitos · editor desplegable DEBAJO del grid**: antes el editor reemplazaba el chip (`gridColumn:'1 / -1'`, estado local) → reordenaba. Ahora `HitosEditor` sube el estado (`activeGoalId`), `GoalRow` es **solo chip** (clic = toggle; chip activo resaltado con borde `T.accent`), y la edición se extrae a un nuevo `GoalEditor` que se renderiza **una vez, debajo del grid**, a ancho completo (mejor sitio para `HousingPathsCard`). Reclicar el chip cierra. Orden de chips intacto. Verificado con la demo (3 hitos).
- **P3 · Menú de cuenta + modal «¿Por qué?»**: en `AccountMenu` se quita «Mis datos» (redundante con la pestaña Datos), «Acerca de» y «Apoyar Mi Plan»; queda una sola entrada **«¿Por qué?»**. `AboutModal` → **`WhyModal`** (`modals/index.jsx`, y refs en `gallery`): título «¿Por qué?» (accent) + «La asignatura que nunca nos dieron» + motivación (educación financiera, abrir los ojos) + qué es (condensado) + **donación visual** (icono café SVG + panel accent + chips Ko-fi/GitHub `· pronto`) + footer AGPL. La presentación/landing sigue accesible desde Datos.
- **No tocado**: motor (`lib`), tokens, `migrateToV2`, claves localStorage, `LEARN_CORPUS`, firmas. Quitar meses futuros vacíos es seguro (`planned` se recalcula vivo, sin refs cruzadas). Cero red (donación = `<a href>`).
- **Verificación**: build OK; `verify-content`/`verify-state` PASS; **verify-lib 13 / verify-tokens 2** (sin diffs nuevos); hash baseline `b3ea52b1…` intacto; consola limpia.

### Barra de «modo demo» con salida
- **Diagnóstico** (reporte del dueño: «se entra directo a Plan+demo en primera visita»): verificado que **no hay bug** — `seedDemo()` solo corre por toque explícito («Ver una demo» en la landing / «Cargar datos demo» / «Saltar · usar demo»); no hay auto-seed ni param `?demo`. Un visitante nuevo ve la landing (confirmado en incógnito). El hueco real: quien toca «ver una demo» queda dentro sin salida clara.
...
### Landing como «home» en cada entrada/refresco
- **Petición del dueño**: que al entrar/refrescar en miplanfire.com se vea SIEMPRE la landing (no caer directo al Plan), con un botón para entrar al plan.
- **Cambio** (`screens/index.jsx` `Shell` + `flows/index.jsx` `LandingPreOnboarding`): puerta de **sesión** `entered` (`useState(false)`, no se persiste → cada carga arranca mostrando la landing). Sustituye al guard `!hasSeenLandingPreOnboarding`. CTA según estado: con datos (`onboardingComplete`) → **«Entrar →»** (`onEnter` → `setEntered(true)`, sin enlace de demo para no sobrescribir); nuevo → **«Empezar →»** (`onStart` → marca `hasSeen`/`landingSeen` + entra → onboarding) + «ver una demo» (`seedDemo` + entra). `LandingPreOnboarding` recibe `returning`/`onEnter`.
- **No tocado**: onboarding, `seedDemo`, persistencia, `migrateToV2`, baseline. Trade-off aceptado: el usuario con plan da un clic «Entrar» por sesión.
- **Verificación**: preview — con datos, recargar → landing con «Entrar →» (sin demo) → al pulsar entra al Plan; sin datos → landing con «Empezar →» + «ver una demo». Build OK; verify-content/state PASS; lib 13 / tokens 2; baseline intacto.

### Ventana «¿De dónde sale el 8%? · ¿En qué se invierte?» (Plan)
- **Petición del dueño**: la gente no ve rápido en qué invertir ni de dónde sale el 8% que asume la app; quería «ventanas» con rentabilidades históricas (fondos mundiales, S&P 500), mostrando opciones sin vender nada.
- **Hallazgo**: el contenido ya existía en `LEARN_CORPUS` (cerrado) pero enterrado tras enlaces `<Concept>` minúsculos; faltaba la capa visual y una puerta visible.
- **Cambio** (`screens/index.jsx`): nueva ventana modal `ReturnsExplainerModal` + gráfico de barras SVG «a mano» `ReturnsBars` + datos estáticos `HISTORICAL_RETURNS` (cero red; nominales, aproximados, largo plazo; alineados con los supuestos internos: efectivo ~0,5 · depósitos ~2,5 · 60/40 ~6 · MSCI World ~8 · S&P 500 ~9,5). El chart marca con línea discontinua el 8% que asume Mi Plan (acaba en la barra MSCI World). La ventana cierra con avisos honestos (pasado≠futuro · caídas −30/−50% · cuenta corriente no da el 8% · educación, no asesoramiento, no vendemos nada) y chips «Profundiza» que **abren los conceptos existentes** (`retorno-anual`, `fondos-indexados`, `asset-allocation`, `volatilidad`, `comisiones`) vía `window.__openLearnConcept` (cero duplicación del corpus).
- **Puertas visibles**: CTA `CartelBtn` en «Dónde estás» (junto al «8% asumido») y en «Hacia dónde puedes ir» → abren la ventana. Estado `showReturns` en `ScreenHoy`.
- **No tocado**: `LEARN_CORPUS`/`content`, motor (`lib`), tokens, `migrateToV2`, claves, baseline. Sin emojis nuevos; color por tokens; cero red.
- **Verificación**: preview (demo) — 2 CTAs en Plan; la ventana muestra el gráfico (efectivo→S&P, línea 8%), filas «qué es», avisos y chips; «Fondos indexados» cierra la ventana y abre su concepto. Build OK; verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline intacto.

### «Antes de Mi Plan»: bloques 1–2 simplificados + cifras cuadradas con el motor
- **Petición del dueño**: la ventana «Antes de Mi Plan» (`SinMiPlanModal` → `ScreenSinMiPlan embedded`, abre con «Ver el cálculo completo →» en Plan) tenía los bloques 1 (Verdad 1 · erosión del salario) y 2 (Verdad 2 · coste de no invertir) que **abrumaban** (cada uno: párrafos + gráfico de líneas Recharts + leyenda + cajas KPI + cifra grande + editable); pedía «di más con menos» y, sobre todo, que **las cifras siguieran la lógica del resto de la app** («me da la sensación de que no cuadra»).
- **Causa raíz del desajuste** (`screens/index.jsx`, `ScreenSinMiPlan`): la ventana NO usaba el motor; recomputaba con `useMemo` propios (`erosion`, `oppCost`) bajo supuestos divergentes — capital `plan.capital` (foto vieja) en vez de `d.currentPortfolio`; crecimiento salarial **1% hardcodeado** en vez de `inflRate × salaryInflationFactor`; retorno **8% fijo** en vez de `d.effectiveReturn`. Además la métrica de erosión sumaba `nominal−real` (grande aunque el sueldo mantuviera poder de compra).
- **Cambio — cifras (todo en UI, reutilizando `useDerived` `d`; `lib` intacto)**:
  - **Verdad 2** apples-to-apples con el dashboard: `investedNominal = d.finalPlan.portfolio` (la proyección oficial `projectV2`, con `effectiveReturn` y eventos), `parkedNominal = (d.currentPortfolio||0) + Σ aportaciones` (mismo capital, mismas aportaciones, 0 %); ambas a € de hoy con `toRealEur(·, monthsToRetire, inflRate)`; `oppDiff = investedReal − parkedReal`. Eliminado el `oppCost` useMemo.
  - **Verdad 1** alineada al motor: el salario nominal crece a `inflRate × (salaryInflationFactor ?? 1)` (no 1 %); pérdida honesta `Σ max(0, income − realActual)` (=0 si el sueldo sigue al IPC → marco honesto «mantienes tu poder de compra»). Quitado el editable `salaryGrowthAnnual` (el dato vive en `salaryInflationFactor`).
  - **Dashboard «Dónde estás» (Card B)**: la tarjeta «Parado» pasa de `sinPlanKPIs.parkedFinalNominal` (capital viejo) a `(d.currentPortfolio||0) + aportadoNominal`, para que Parado/Invertido sean comparables (misma base que la ventana).
- **Cambio — visual («di más con menos»)**: fuera los gráficos de líneas Recharts (`ErosionChart`/`OppCostChart`), leyendas y párrafos de más. Nuevo `CompareBars` (dos barras cartel SVG a mano, tokens): V1 «Hoy» vs «Dentro de N años · poder de compra» + cifra grande **−{lostAccum}**; V2 «Parado · cuenta corriente» vs «Invertido · Mi Plan» + cifra grande **+{oppDiff}** en verde (ganancia, no alarma) + `<Concept id="interes-compuesto">`. Bloques 3–5, CTA y `ActualLifeOnboarding` intactos; `embedded` respeta el título del modal.
- **No tocado**: `computeSinPlanKPIs`/`projectV2`/`runMonteCarlo` (`lib`), `LEARN_CORPUS`/`content`, tokens, `migrateToV2`, claves, baseline. Cero red; color por tokens; sin emojis nuevos.
- **Verificación**: preview (demo) — V1 y V2 limpios (sin línea Recharts, sin editable). Cruce de cifras: dashboard nominal Parado 320k€ / Invertido 1,18 M€ → ventana real (× mismo deflactor 0,453) Parado 145k€ / Invertido 535k€ (ratio 3,69 idéntico en nominal y real); ganancia +390k€ = 535−145. Cuadran con el motor. Build OK; verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; consola limpia.

### «Antes de Mi Plan» (ronda 2): la ventana sigue el modo de la app + bloques 3 y 5 a barra segmentada
- **Petición del dueño**: dos cosas seguían mal. (1) «¿Qué sentido tiene que ponga ~422 cuando le decimos que invertido son 1,18 M?» — la ventana forzaba «€ de hoy» (Verdad 2 = 535k) e ignoraba el modo de la app, y **Verdad 3 tenía una caja que proyectaba el sobrante mensual a 8 % nominal → ≈419k€**, una tercera cifra de «invertir» en otra base que chocaba con el 1,18 M de la cabecera. (2) Verdad 3 (gastos) y Verdad 5 (patrimonio) seguían siendo **tablas de filas + párrafos + jerga** («rinde X% nominal · Y% real», «si movieras todo el efectivo…»): pedía menos texto, más visual.
- **Decisiones (AskUserQuestion)**: la cifra → **«seguir el modo de la app»**; los bloques → **«una barra segmentada + 1 frase»**.
- **Cambio — coherencia de la cifra** (`screens/index.jsx`, `ScreenSinMiPlan`, solo UI): la cabecera (`d.finalPlan.portfolio`) y el dashboard son **nominales por defecto** (modo `'nominal'`), pero la ventana ignoraba el `realMode` que ya calculaba. Ahora **Verdad 2 sigue `realMode`** igual que «Hacia dónde» (`realMode ? finalReal : finalNominal`): `invertidoView = realMode ? investedRealSMP : investedNominalSMP` (= **1,18 M** en nominal, idéntico a la cabecera; su equivalente en € de hoy en real), `paradoView`/`oppDiffView` análogos. Subtítulo mode-aware («Dentro de N años.» / «En N años, en € de hoy.»). **Eliminada la caja «sobrante → 419k»** (8 % nominal hardcodeado; su mensaje ya lo da Verdad 2). Verdad 1 (poder de compra) intacta.
- **Cambio — visual («menos texto, más imágenes»)**: nuevo `StackedBar` (barra apilada horizontal cartel + leyenda compacta en `T.mono`, reutiliza el patrón de rayado existente). **Verdad 3**: una barra (gastos `T.muted` · ahorro/inversión `T.accent` · sobrante `T.amber` rayado) + 1 frase; fuera las 7 filas-barra y los 2 párrafos. **Verdad 5**: una barra coloreada por rentabilidad (rojo <1 % · amber 1-3 % · verde >3 %, ya en `c.color`) + 1 frase con `d.effectiveReturn` nominal/real; fuera la jerga por fila y el párrafo «si movieras…».
- **No tocado**: `lib` (`computeSinPlanKPIs`/`projectV2`/`runMonteCarlo`), `content`/`LEARN_CORPUS`, tokens, `migrateToV2`, claves, baseline. Verdad 4 (hipoteca, condicional) y CTA intactos. La cabecera y las tarjetas «Dónde estás · N años» siguen siendo nominales-siempre (comportamiento preexistente, fuera de alcance).
- **Verificación**: preview (demo). Nominal (por defecto): ventana Invertido **1,18 M** = cabecera = dashboard «Hacia dónde»; ganancia +859k€; **no aparece ninguna cifra 419/422** (`has419:false`) ni jerga vieja (`hasOldJargon:false`). Real (toggle «€ de hoy»): ventana Invertido **535k** = «Hacia dónde» real (deflactado); subtítulo «en € de hoy». V3/V5 = una barra + leyenda + una frase. Build OK (1.095,80 kB); verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; consola limpia.

### «Antes de Mi Plan» (ronda 3): tipografía, espaciado y color de los bloques de barra segmentada
- **Petición del dueño**: «cuesta identificar lo que es cada elemento» en los bloques de barra segmentada (Verdad 3 gastos y Verdad 5 patrimonio); mejorar tipografía, espaciado y color.
- **Restricción (doctrina)**: «color solo por tokens» — prohibido dar un color propio a cada categoría o usar rampas de opacidad; el color es semántico. Por eso la identificación se resuelve por la **leyenda**, no por el color. Decisiones (AskUserQuestion): **leyenda clara, color semántico** + barra de gastos **agrupada en 3 zonas**.
- **Cambio** (`screens/index.jsx`, solo UI): `StackedBar` rediseñado. Leyenda antes en `T.mono` `eyebrow` (11px) en línea/envuelta `swatch · etiqueta · € · %` → ahora **lista en grid alineado** (`gridTemplateColumns: '1fr auto auto'`, `rowGap` 10, `marginTop` 18): swatch (cuadrado redondeado 11px, rayado si `dashed`) + **etiqueta `T.serif` 15px `T.ink`** + **€ `T.mono` 13px `T.ink` tabular** + **% `T.mono` 13px `T.faint` tabular**, con `fontVariantNumeric: 'lining-nums tabular-nums'` para alinear columnas. Barra con más cuerpo (altura 26, `border` `T.line`, separadores 2px `T.paper`); helper local `stripe(a,b)` para el rayado (no deja el `repeating-linear-gradient` suelto). Nueva prop opcional `barSegments` para agrupar la barra sin tocar el detalle de la leyenda.
- **Verdad 3**: barra agrupada en **3 zonas** (`Gastos` `T.muted` · `Ahorro/inversión` `T.accent` · `Sobrante` `T.amber` rayado) vía `barSegments`; la leyenda mantiene el detalle de 7 categorías. **Verdad 5**: el código de color del subtítulo («rojo <1% · amber 1-3% · verde >3%») pasa de texto a **3 puntos** en `T.red`/`T.amber`/`T.green`, ligando color↔significado; la barra/leyenda siguen coloreadas por rentabilidad (la coincidencia de dos verdes es correcta: ambos = buena rentabilidad, la etiqueta los distingue).
- **No tocado**: datos/cifras (la leyenda no recalcula nada), `lib`, `content`, tokens, `migrateToV2`, claves, baseline. Verdad 1 y 2 (`CompareBars`) intactos. Sin colores nuevos (solo tokens), sin emojis nuevos.
- **Verificación**: preview (demo) — V3 y V5 con leyenda alineada legible (swatch + nombre serif + € y % en columnas tabulares), barra de gastos en 3 zonas, puntos de color en patrimonio. Build OK (1.096,97 kB); verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; consola limpia.

### Plan · «Supuestos» como nota difuminada + «Ver el cálculo completo» como card-ventana
- **Petición del dueño**: en el dashboard de Plan, cambiar los «Supuestos» desplegables (con «+») por una **nota/card difuminada** (a la vista, sin abrir); y el enlace de texto «Ver el cálculo completo →» por una **card tipo ventana difuminada** que incite a abrirla.
- **Cambio** (`screens/index.jsx`, solo UI): dos componentes nuevos. **`NotaSupuestos`** — footnote difuminada (`T.faint`, eyebrow «SUPUESTOS» + texto serif itálico en `T.size.caption`, fondo `T.bg`); sustituye los dos `OnboardingHelp title="Supuestos"` de «Dónde estás» y «Hacia dónde» (el `OnboardingHelp` genérico se mantiene para las preguntas «¿Por qué…?» del onboarding). **`CalculoCompletoCard`** — card clicable con cabecera de ventana («Antes de Mi Plan · cálculo completo» + «abrir ↗»), un vistazo **borroso** (`filter: blur(2px)`) del contenido real («Tu situación si no haces nada.» + barras fantasma neutras) bajo un velo que funde a `T.paper`, con la CTA «Ver el cálculo completo →» nítida encima y lift en hover; abre `SinMiPlanModal`. Sustituye el `CartelBtn variant="text"`.
- **No tocado**: el modal y su contenido, motor/`lib`, `content`, tokens, `migrateToV2`, claves, baseline. Color solo por tokens (los grises del vistazo son `T.ink`/`T.muted` a baja opacidad; el velo usa `transparent → T.paper` como el patrón ya existente); sombras `rgba(26,22,18,…)` como en `BookPromo`. Sin emojis nuevos (· `↗`/`→` ya en uso).
- **Verificación**: preview (demo) — en «Dónde estás» y «Hacia dónde» los «Supuestos» son notas difuminadas (sin «+»); la card-ventana muestra el vistazo borroso + CTA y **abre el modal** al pulsar; sin overflow en 414px. Build OK (1.099,78 kB); verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; consola limpia.

### Plan · ventanas abren por su inicio (portal) + card difuminada del 8 % (solo bloque 1)
- **Petición del dueño**: (1) que cualquier desplegable/ventana se abra por su **inicio**, no a mitad teniendo que subir; (2) aplicar la card «ventana difuminada» al CTA «¿De dónde sale ese 8 %?» y que **solo aparezca en el bloque 1** («Dónde estás»), quitándolo del bloque 2 («Hacia dónde»).
- **Causa raíz (1)** (`screens/index.jsx`): el componente `Reveal` (`ui/cartel.jsx`) lleva `transform`/`willChange`; en CSS eso convierte al ancestro en el bloque contenedor de los `position: fixed` descendientes → un modal renderizado dentro de un `Reveal` se posiciona respecto a esa caja (a mitad), no al viewport. Los modales de `modals/index.jsx` ya salen por `Portal` (inmunes); los inline `SinMiPlanModal`, `ReturnsExplainerModal` y `ActualLifeOnboarding` (rama modal) no.
- **Cambio (1)**: portar esos tres a `document.body` (`createPortal`, ya importado) y, como garantía, `const scrollRef = useRef(null)` + `useEffect(() => scrollRef.current?.scrollTo(0,0), [])` con `ref` en el overlay con `overflowY:auto`. `ActualLifeOnboarding` solo se porta en modo modal (`!inline`; la rama `inline` se devuelve tal cual). Verificado: tras scrollear la página, abren con `overlayScrollTop 0` y su título a ~60–98 px del top del viewport.
- **Cambio (2)**: generalizada la card en **`VentanaDifuminadaCard({ eyebrow, ctaLabel, onOpen, mobile, children })`**; `CalculoCompletoCard` pasa a ser wrapper sobre ella (sin cambios visibles), y nuevo **`Returns8Card`** (eyebrow «El motor · rentabilidad histórica», vistazo borroso que evoca `ReturnsBars` con barras ascendentes en grises de token, CTA «¿De dónde sale ese 8 %? ¿En qué se invierte?»). En bloque 1 reemplaza el `CartelBtn` del 8 %; en bloque 2 ese CTA se **elimina** (se conservan la `NotaSupuestos` y «Profundizar en Proyección»).
- **No tocado**: `modals/index.jsx` (ya portan), motor/`lib`, `content`/`LEARN_CORPUS`, tokens, `migrateToV2`, claves, baseline. Color solo por tokens; sin emojis nuevos; firmas y `localStorage` intactos.
- **Verificación**: preview (demo) — los 3 modales portados a `body` y abren por su inicio aunque la página esté scrolleada; en bloque 1 la card del 8 % (abre la ventana de rentabilidades), en bloque 2 ya no está ese CTA; 1 sola aparición del 8 % en todo Plan. Build OK (1.100,67 kB); verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; consola limpia.

### Verdad 4 (colores hipoteca) + SEO indexable
- **Petición del dueño**: (1) en la demo de Marta, las barras de la Verdad 4 (hipoteca) «parecen bugeadas»; (2) que la web sea encontrable al buscar «mi plan fire». (3) panel de sugerencias: se deja listo (ya se activa con `SUGGEST_URL`) + guía aparte para crear el formulario — sin cambio de código.
- **Causa raíz (1)**: `MortgageChart` apilaba `principal` en `T.accent` (#c2410c) e `interest` en `T.amber` (#b45309), casi el mismo naranja → barras fundidas. **Cambio** (`screens/index.jsx`): `principal` pasa a `T.muted` (neutro, recede) e `interest` se queda en `T.amber` (el «coste oculto» destaca); leyenda actualizada. Verificado con Marta: gris (capital) + naranja (intereses), patrón de amortización legible.
- **Cambio (2) · SEO on-page** (`app/index.html` + `app/public/`, estático, cero red): **quitado** `<meta name="robots" content="noindex">` (era el freno de indexado); `<title>` «Mi Plan FIRE · Independencia financiera y retiro temprano»; `meta description`; `canonical` a `https://app.miplanfire.com/`; Open Graph (`og:type/site_name/title/description/url/locale`); Twitter card `summary`; `theme-color #f5f0e6`; **favicon** `public/favicon.svg` (marca: dos barras crema sobre `T.accent`); **JSON-LD** `WebApplication` («Mi Plan FIRE», FinanceApplication, gratis, es). Nuevos `public/robots.txt` (+ Sitemap) y `public/sitemap.xml`. `og:image` aplazado (requiere PNG 1200×630; no afecta al ranking de marca).
- **No tocado**: motor/`lib`, `content`, tokens, `modals/index.jsx` (`SUGGEST_URL` sigue vacío hasta tener la URL del formulario), `migrateToV2`, claves, baseline. Acciones pendientes del dueño (fuera de código): crear el formulario de sugerencias y verificar/enviar sitemap en Google Search Console.
- **Verificación**: build OK; `dist/` con `index.html` (sin noindex, con title/description/OG/JSON-LD/favicon), `robots.txt`, `sitemap.xml`; Verdad 4 de Marta con colores nítidos; consola limpia; verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto.
...
### Datos · «tu situación económica» en 3 desplegables (acordeón)
- **Petición del dueño**: colapsar por títulos el editor de «tu situación económica» (en Datos) en tres bloques desplegables.
- **Cambio** (`screens/index.jsx`): nuevo componente `InlineFold` (cabecera con título + resumen + chevron «▸ ver/▾ ocultar», cuerpo colapsable, cerrado por defecto). En `ActualLifeOnboarding`, las tres secciones del modo **inline** (Gastos / Hipoteca / Dónde está tu dinero) se envuelven en `InlineFold` solo cuando `inline` (patrón `inline ? <InlineFold>{content}</InlineFold> : content`); el título-pregunta del wizard se oculta en inline (`{!inline && …}`) porque la cabecera del fold ya lo nombra. Resúmenes en la cabecera: Gastos `{total}/mes`, Hipoteca `Sí/No`, Dónde está tu dinero `{total}%`.
- **Wizard intacto**: en modo modal (`!inline`, onboarding y «Editar mis datos») se devuelve el `content` con su título de paso — sin folds. Verificado: «Paso 1 de 4» + «¿Cuánto te cuesta vivir cada mes?» siguen saliendo.
- **No tocado**: motor/`lib`, `content`, tokens, claves, baseline. (Aprovechando, se quitaron en Datos los botones Exportar/Importar JSON y CSV; ver entrada anterior.)
- **Verificación**: preview (demo, Datos) — 3 folds cerrados (Gastos 1.6k€/mes · Hipoteca Sí · Dónde está tu dinero 100%); al expandir Gastos aparecen sus filas + subtítulo; el wizard modal sigue mostrando título y «Paso 1 de 4». Build OK; verify-content/state PASS; lib 13 / tokens 2 (sin diffs nuevos); baseline `b3ea52b1…` intacto; módulo recargado sin errores.
